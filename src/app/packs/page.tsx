...
export default function PacksPage() {
  ...
  const [lastOpenedPack, setLastOpenedPack] = useState<{ packId: string; quantity: number } | null>(null);
    ...
    try {
      ...
      if (data.success) {
        ...
        if (data.data.lastDailyCoins) {
          ...
          if ((Date.now() - lastClaim) / (1000 * 60 * 60) < 24) setDailyClaimed(true);
        }
      }
  ...
  useEffect(() => { fetchUser(); fetchPacks(); }, [fetchUser, fetchPacks]);
  ...
  function handleOpenAnother() {
    if (lastOpenedPack) {
      setRevealedCards(null);
      handleOpenPack(lastOpenedPack.packId, lastOpenedPack.quantity);
    }
  }
  ...
  async function handleClaimDaily() {
    try {
      ...
      } else { setError(data.error || 'Failed to claim daily coins'); }
    } catch (err) { console.error('Failed to claim daily coins:', err); setError('Failed to claim daily coins'); }
  }
        ...
        <main className="max-w-6xl mx-auto px-4 py-8">
          <div className="flex justify-between items-center mb-8 flex-wrap gap-3">
            ...
            <div className="flex gap-2 flex-wrap">
              ...
              <button onClick={handleClaimDaily} disabled={dailyClaimed}
                ...
                style={{ background: dailyClaimed ? '#374151' : 'linear-gradient(135deg, #FACC15, #EAB308)', color: dailyClaimed ? '#9CA3AF' : '#000', boxShadow: dailyClaimed ? 'none' : '0 4px 16px rgba(234,179,8,0.3)' }}>
          ...
          {loading ? (
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
              {packs.map((pack) => (
                <PackCard key={pack.id} pack={pack} onOpen={handleOpenPack} userCoins={user?.coins ?? 0} />
              ))}
            </div>
          )}
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ background: 'radial-gradient(ellipse at 50% 40%, #1a1a2e 0%, #0a0a15 100%)' }}>
              <div className="text-center">
                <div className="relative w-20 h-20 mx-auto mb-6">
                  <div className="absolute inset-0 rounded-full" style={{ border: '4px solid rgba(255,255,255,0.1)', borderTopColor: '#8B5CF6', animation: 'spin 0.8s linear infinite' }} />
